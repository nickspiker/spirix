// spirix_alu_addbit — combinational add/sub + bitwise ALU
//
...
// Floor-only — barrel shift truncates, no rounding.
// Single-stage combinational with output register only.
//
...
module spirix_alu_addbit #(
    ...
    // ================================================================
    //  COMBINATIONAL FINISH (no pipeline register — single-stage)
    // ================================================================

    // Alias s2_ names to S1 wires for minimal diff with pipelined version
    wire [MAX_FRAC-1:0]        s2_close_normalized = close_normalized;
    wire                       s2_close_ovf        = close_ovf;
    wire signed [MAX_FRAC-1:0] s2_close_ovf_frac   = close_ovf_frac;
    wire [LEAD_BITS-1:0]       s2_close_leading    = close_leading;
    wire                       s2_close_is_zero    = close_is_zero;
    wire signed [MAX_FRAC-1:0] s2_far_aligned      = $signed(b5_val);
    wire signed [MAX_FRAC-1:0] s2_big_frac         = big_frac;
    wire signed [MAX_EXP-1:0]  s2_big_ei           = big_ei;
    wire                       s2_is_close         = is_close;
    wire                       s2_negligible       = negligible;
    wire                       s2_negate_big       = negate_big;
    wire                       s2_negate_small     = negate_small;
    wire                       s2_sub              = is_sub;
    wire                       s2_shortcut         = shortcut;
    wire signed [MAX_FRAC-1:0] s2_sc_frac          = sc_frac;
    wire signed [MAX_EXP-1:0]  s2_sc_ei            = sc_ei;
    wire                       s2_bypass_add       = bypass_add;
    wire                       s2_bypass_sub       = bypass_sub;
    wire signed [MAX_FRAC-1:0] s2_neg_big_frac     = neg_big_frac;
    wire signed [MAX_EXP-1:0]  s2_neg_big_ei       = neg_big_ei;
    wire                       s2_bw_negligible    = bw_negligible;
    wire signed [MAX_FRAC-1:0] s2_bw_negl_frac     = bw_negl_frac;
    wire signed [MAX_EXP-1:0]  s2_bw_negl_ei       = bw_negl_ei;
    wire [2:0]                 s2_op               = op;
    wire [1:0]                 s2_frac_width       = frac_width;
    wire [1:0]                 s2_exp_width        = exp_width;
    wire [6:0]                 s2_frac_bits        = frac_bits;
    wire signed [MAX_EXP-1:0]  s2_small_ei         = small_ei;

    wire s2_is_addsub  = is_addsub;
    wire s2_is_and     = is_and;
    wire s2_is_or      = is_or;
    wire s2_is_xor     = is_xor;
    wire s2_is_bitwise = is_bitwise;
