...
module spirix_addsub_natwrap #(
    ...
    // New cyclic-modular encoding: AMBIG at all-zeros, MAX/MIN adjacent to it via wrap.
    ...
    wire signed [EXP_BITS-1:0] sc_exp =
        sc_a_undef       ? a_exp :
        sc_b_undef       ? b_exp :
        sc_inf           ? AMBIG_EXP[EXP_BITS-1:0] :
        sc_a_zero        ? (sub ? neg_b_exp : b_exp) :
        sc_b_zero        ? a_exp :
        sc_exp_exp       ? AMBIG_EXP[EXP_BITS-1:0] :
        sc_van_van       ? AMBIG_EXP[EXP_BITS-1:0] :
        sc_a_exp_b_van   ? a_exp :
        sc_a_van_b_exp   ? (sub ? neg_b_exp : b_exp) :
        sc_a_exp         ? AMBIG_EXP[EXP_BITS-1:0] :
        sc_b_exp         ? AMBIG_EXP[EXP_BITS-1:0] :
    ...
    wire signed [EXP_BITS:0] raw_diff = $signed({1'b0, a_exp}) - $signed({1'b0, b_exp});
    ...
    // Free overflow: cyclic-modular wrap to AMBIG=0 catches +1 overshoot.
    // Underflow needs wide compare (cancellation can drop by FRAC binades).
    wire underflow = (exp_calc < $signed({{(ECW-EXP_BITS){1'b0}}, MIN_EXP}));
    wire overflow  = (out_exp == AMBIG_EXP) & ~underflow;
    ...
    assign result_exp  = shortcut                  ? sc_exp :
                         ...
                         (is_zero_ext | overflow | underflow) ? AMBIG_EXP[EXP_BITS-1:0] :
                                                                out_exp;
