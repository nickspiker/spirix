        ...
        for ea in -128i8..=127 {
            ...
            for fb in -128i8..=127 {
                for eb in -128i8..=127 {
                    ...
                    if !matches!(cb, Class::Vanished | Class::Exploded) {
                        continue;
                    }
                    let r = a / b;
                    let cr = classify(r);
                    ...
                    if cr != expected {
                        if cb == Class::Vanished {
                            miss_norm_van_nonxpl += 1;
                        }
                        if cb == Class::Exploded {
                            miss_norm_xpl_nonvan += 1;
                        }
                        if shown < 6 {
                            println!("--- a/b ---");
                            show("a", a);
