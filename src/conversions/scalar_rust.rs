    ...
    I256: From<E>,
{
    fn into(self) -> f64 {
        if self.is_normal() {
            ...
            let exponent: i32 = self.v01_exp().saturate();
...
fn into(self) -> $i {
    ...
    let exp: isize = self.v01_exp().saturate();
    ...
    ($frac:ty, $exp:ty, $frac_bits:expr) => {
        impl Scalar<$frac, $exp> {
            ...
            fn _to_f32_legacy(&self) -> f32 {
                ...
                let raw_exp_i = (self.v01_exp() as i32) + 126 + exp_adj;
                if raw_exp_i >= 255 {
    ...
    ($frac:ty, $exp:ty, $frac_bits:expr) => {
        impl Scalar<$frac, $exp> {
            ...
            fn _to_f64_legacy(&self) -> f64 {
                ...
                let raw_exp_i = (self.v01_exp() as i64) + 1022 + exp_adj;
                if raw_exp_i >= 2047 {
                    return if sign_bit != 0 {
                        f64::NEG_INFINITY
                    } else {
