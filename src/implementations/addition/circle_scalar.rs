        ...
        if self.exponent > scalar.exponent {
            ...
            match F::FRACTION_BITS {
                8 => {
                    ...
                    if self.exponent.is_negative() && !offset.is_negative() {
                        return Self {
                            real: ((result_r << (leading - 2)) >> F::FRACTION_BITS).as_(),
                            imaginary: ((big_i << (leading - 2)) >> F::FRACTION_BITS).as_(),
                            exponent: E::AMBIGUOUS_EXPONENT,
                        };
                    }

                    return Self {
                        real: ((result_r << (leading - 1)) >> F::FRACTION_BITS).as_(),
                        imaginary: ((big_i << (leading - 1)) >> F::FRACTION_BITS).as_(),
                ...
                128 => {
                    ...
                    if self.exponent.is_negative() && !offset.is_negative() {
                        return Self {
                            real: ((result_r << (leading - 2)) >> F::FRACTION_BITS)
                                .as_i128()
                                .as_(),
                            imaginary: ((big_i << (leading - 2)) >> F::FRACTION_BITS)
                                .as_i128()
                                .as_(),
                            exponent: E::AMBIGUOUS_EXPONENT,
                        };
                    ...
                    return Self {
                        real: ((result_r << (leading - 1)) >> F::FRACTION_BITS)
                            .as_i128()
                            .as_(),
                        imaginary: ((big_i << (leading - 1)) >> F::FRACTION_BITS)
                            .as_i128()
                            .as_(),
                        exponent: offset + 1.as_(),
                    };
            ...
            match F::FRACTION_BITS {
                8 => {
                    ...
                    let result_r = big_r + small_r;
                    if result_r.is_zero() && self.imaginary == 0.as_() {
                        return Self::ZERO;
                    }
                    let leading_r = result_r.leading_ones().max(result_r.leading_zeros()) as isize;
                    ...
                    if scalar.exponent.is_negative() && !offset.is_negative() {
                        return Self {
                            real: ((result_r << (leading - 2)) >> F::FRACTION_BITS).as_(),
                            imaginary: ((result_i << (leading - 2)) >> F::FRACTION_BITS).as_(),
                            exponent: E::AMBIGUOUS_EXPONENT,
                        };
                    }
                    return Self {
                        real: ((result_r << (leading - 1)) >> F::FRACTION_BITS).as_(),
                        imaginary: ((result_i << (leading - 1)) >> F::FRACTION_BITS).as_(),
                        exponent: offset + 1.as_(),
                    };
                }
                16 => {
                ...
                128 => {
                    ...
                    let result_r = big_r + small_r;
                    if result_r == 0.into() && self.imaginary == 0.as_() {
                        return Self::ZERO;
                    }
                    let leading_r = result_r.leading_ones().max(result_r.leading_zeros()) as isize;
