...
\ppara{Second, arithmetic operations (addition, subtraction, multiplication, division) operate directly on the two's complement fractions without sign extraction, sign testing, or sign-dependent branching.}
...
\ppara{On identical silicon (Lattice ECP5-25F) with Yosys/nextpnr synthesis, the two implementations measure as shown in FIG. \ref{fig:silicon_comparison}. Spirix figures are measured maximum operating frequency on silicon via a clock-enable-gated self-test harness; HardFloat figures are measured on the same silicon using the same harness. Both sides include full spec-compliant edge-case handling.}

\begin{lstlisting}[caption={Silicon area and frequency measurement on Lattice ECP5-25F, binary32-equivalent precision. Measured maximum operating frequency via CE-gated self-test harness. HardFloat areas include fNToRecFN/recFNToFN converters (full IEEE I/O); core-only areas are 546 / 282 / 1344 LUT4 for add/mul/FMA respectively}, label={fig:silicon_comparison}]
                       Invention               Berkeley HardFloat
Operation       LUT4   DSP   MHz         LUT4   DSP   MHz
-----------------------------------------------------------------
Add / Subtract  842    0     95          1050   0     88
Multiply        227    4     115         786    4     65
FMA             1472   3     63          2057   4     47
\end{lstlisting}

\ppara{FIG. \ref{fig:scalar_subtract} presents the synthesizable Verilog implementation of Scalar add/subtract using the invention's two's complement N0 method, silicon-verified at F4E3 precision. A single combinational datapath handles every normal case — no sign bit test, no denormal path, no implied-one manipulation — and a flat shortcut network handles the abnormal operand combinations. Silicon measurements for both implementations on identical hardware are given in FIG. \ref{fig:silicon_comparison}.}
...
\ppara{- \textbf{No sign field:} The fraction is a two's complement integer with no sign field. Sign is inherent in the two's complement representation. All fraction bits contribute directly to the value, and arithmetic operates on the fractions without sign extraction or sign-dependent branching.}
...
\begin{enumerate}
              ...
              \item a memory storing instructions which, when executed by the processor, implement a floating-point arithmetic system that:
                    \begin{enumerate}
                        ...
                        \item encodes multiple distinct non-normal states at the ambiguous exponent, differentiated by the leading same bit count of the fraction component, including:
                              ...
                              \end{enumerate}
                        \item performs basic arithmetic operations (add, subtract, multiply, divide) directly on the two's complement fractions without sign extraction or sign-dependent branching;
    ...
    \item The system of claim 1, wherein the two's complement fraction representation operates such that:
          \begin{enumerate}
              \item every possible fraction bit pattern is valid for normal values, with no encoding space reserved for sign indication;
              \item for an $n$-bit fraction, there are $2^n$ distinct normal fractions per exponent value; and
              \item sign is inherent in the two's complement representation and requires no dedicated field or extraction; basic arithmetic operations (add, subtract, multiply, divide) operate on the fraction without sign testing or sign-dependent branching.
          \end{enumerate}

    \item The system of claim 1, wherein the Scalar representation defines a complete state partition: every one of the $2^{F+E}$ distinct (fraction, exponent) bit patterns maps to exactly one defined state, comprising:
          \begin{enumerate}
              \item a normal value at every non-ambiguous exponent paired with any fraction pattern; and
              \item one of the multi-state non-normal classifications (exploded, vanished, zero, infinity, undefined) at the ambiguous exponent, determined by the leading same bit count of the fraction,
          \end{enumerate}
          such that no (fraction, exponent) bit pattern is reserved, unused, or malformed.
    ...
    \item A method for processing floating-point values in a computing system, comprising:
          \begin{enumerate}
              ...
              \item encoding multiple distinct non-normal states at the ambiguous exponent by utilizing the leading same bit count of the fraction to differentiate between:
                    ...
                    \end{enumerate}
              \item performing basic arithmetic operations (add, subtract, multiply, divide) directly on the two's complement fractions without sign extraction or sign-dependent branching;
    ...
    \item The method of claim 6, wherein the two's complement fraction representation eliminates sign-dependent branching during basic arithmetic operations (add, subtract, multiply, divide), as:
          \begin{enumerate}
              \item addition and subtraction operate directly on two's complement fractions without testing or branching on sign;
              \item multiplication and division operate on two's complement fractions with the result's sign determined naturally by two's complement arithmetic; and
              \item no sign extraction, sign testing, or sign-magnitude conversion is required for basic arithmetic operations.
          \end{enumerate}
    ...
    \item The method of claim 6, wherein basic arithmetic operations (add, subtract, multiply, divide) on the two's complement floating-point representation proceed without sign-bit testing, sign-dependent branching, denormal handling, or implied-one manipulation, characterized by:
          \begin{enumerate}
...
\end{lstlisting}
\begin{center}
    \fbox{\includegraphics[width=0.9\textwidth]{addition truth table.pdf}}
\end{center}
...
\end{lstlisting}
\begin{center}
    \fbox{\includegraphics[width=0.9\textwidth]{subtraction truth table.pdf}}
\end{center}
...
\end{lstlisting}
\begin{center}
    \fbox{\includegraphics[width=0.9\textwidth]{multiplication truth table.pdf}}
\end{center}
...
\end{lstlisting}
\begin{center}
    \fbox{\includegraphics[width=0.9\textwidth]{division truth table.pdf}}
\end{center}
...
\end{lstlisting}
\begin{center}
    \fbox{\includegraphics[page=1,height=0.85\textheight,keepaspectratio]{Scalar subtract flowchart v1.pdf}}
\end{center}
\newpage

\begin{lstlisting}[style=verilog, caption={Verilog Implementation of IEEE-754 Binary32 Add/Subtract (Berkeley HardFloat \texttt{addRecFN}, BSD-3-Clause, John R. Hauser, 2019). Core add module only; the full IEEE round-trip also requires \texttt{fNToRecFN} and \texttt{recFNToFN} converters plus helper modules (\texttt{recFNToRawFN}, \texttt{isSigNaNRecFN}, \texttt{compressBy2}, \texttt{compressBy4}, \texttt{countLeadingZeros}, \texttt{lowMaskHiLo}, \texttt{roundRawFNToRecFN}, \texttt{propagateFloatNaN\_add})}, label={fig:ieee_subtract}]
// Berkeley HardFloat addRecFN — IEEE-754 add/sub at binary32 precision
// (expWidth=8, sigWidth=24). Operates on the internal "recFN" recoded
// format; inputs/outputs must be translated to/from IEEE-754 binary32
// via the fNToRecFN / recFNToFN converters at module boundaries.
...
module addRecFNToRaw #(
    ...
    // --- Unpack a: recoded IEEE → raw sign/exp/sig + class flags
    ...
    // --- Unpack b (and apply subOp by flipping sign of b)
    ...
    // --- CLOSE path: |Δexp| ≤ 1, subtract, may totally cancel
    ...
    // --- FAR path: |Δexp| ≥ 2, right-shift smaller, add or subtract
    ...
    // --- Special-case plumbing: NaN, Inf, signed zero propagation
...
`ifdef HardFloat_propagateNaNPayloads
    ...
    propagateFloatNaN_add #(sigWidth)
        propagateNaN(control, subOp, isNaNA, signA, sigA[(sigWidth - 2):0],
                     isNaNB, signB, sigB[(sigWidth - 2):0], signNaN, fractNaN);
...
// Top-level add wrapper: addRecFNToRaw → roundRawFNToRecFN
module addRecFN #(
    parameter expWidth = 3, parameter sigWidth = 3
)(
    input  [(`floatControlWidth - 1):0] control,
    ...
    addRecFNToRaw #(expWidth, sigWidth)
        addRecFNToRaw(control, subOp, a, b, roundingMode,
                      invalidExc, out_isNaN, out_isInf, out_isZero,
                      out_sign, out_sExp, out_sig);
    roundRawFNToRecFN #(expWidth, sigWidth, `flRoundOpt_subnormsAlwaysExact)
        roundRawOut(control, invalidExc, 1'b0, out_isNaN, out_isInf,
                    out_isZero, out_sign, out_sExp, out_sig,
                    roundingMode, out, exceptionFlags);
endmodule
\end{lstlisting}
