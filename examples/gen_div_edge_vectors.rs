...
fn verilog_div_result(af: i8, ae: i8, bf: i8, be: i8) -> (i8, i8) {
    ...
    if a_is_ambig || b_is_ambig {
        ...
        if is_undef(bf) { return (bf, be); }
        if b_is_ambig && bf == 0 {
            if a_is_ambig && af == 0 { return (UNDEF_NEG_DIV_NEG, AMBIG); }
            return (-1, AMBIG); // infinity
        }
        if a_is_ambig && af == -1 {
            if b_is_ambig && bf == -1 { return (UNDEF_TF_DIV_TF, AMBIG); }
            return (-1, AMBIG);
        }
        ...
        if a_exploded && b_exploded { return (UNDEF_TF_DIV_TF, AMBIG); }
        if a_vanished && b_vanished { return (UNDEF_NEG_DIV_NEG, AMBIG); }
        if a_vanished || b_vanished { return (UNDEF_GENERAL, AMBIG); }
        // exploded || exploded, or remaining AMBIG → GENERAL
        return (UNDEF_GENERAL, AMBIG);
    }
    if bf == 0 { return (-1, AMBIG); }
    // Normal — skip
    (0x7F, 0x7F) // sentinel
}
