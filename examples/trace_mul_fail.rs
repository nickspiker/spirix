...
fn main() {
        ...
        for e1 in -128i8..=127 {
            ...
            for f2 in -128i8..=127 {
                for e2 in -128i8..=127 {
                    ...
                    let r = s2 * s1;
                    let rc = class(r);
                    if rc == "[0]" && found_zero.is_none() { found_zero = Some((s1, s2)); }
                    if rc == "[∞]" && found_inf.is_none() { found_inf = Some((s1, s2)); }
                    if rc == "[℘?]" && found_und.is_none() { found_und = Some((s1, s2)); }
    ...
    for (name, opt) in [("ZERO", found_zero), ("INFTY", found_inf), ("UNDEF", found_und)] {
        if let Some((s1, s2)) = opt {
            ...
            let fr: f64 = r.into();
            println!("{}: s2*s1 = [{:#04x},{}] * [{:#04x},{}]  = [{:#04x},{}]",
                     name, b2[0] as u8, b2[1], b1[0] as u8, b1[1], br[0] as u8, br[1]);
            println!("      f64: {:.3e} * {:.3e} = {:.3e}  (spirix: {} = {:.3e})",
                     f2, f1, f2*f1, class(r), fr);
        } else {
