...
fn expected_class_from_f64(r: f64) -> Option<Class> {
    if r.is_nan() {
        return Some(Class::Undefined);
    }
    if r.is_infinite() {
        return Some(Class::Infinity);
    }
    let abs = r.abs();
    if abs == 0.0 {
        return Some(Class::Zero);
    }
    ...
    if abs < MIN_NORMAL {
        return Some(Class::Vanished);
    }
    if abs > MAX_NORMAL {
        return Some(Class::Exploded);
    }
    Some(Class::Normal)
}
...
fn run(o: Op) {
    ...
    for ((spirix_c, expected_c), count) in &confusions {
        ...
        let f2: f64 = s2.into();
        println!(
            "  spirix says {}, f64 says {}  ({} cases)  e.g.  {} {} {} = {} (spirix) / f64={:.3e}",
            name(*spirix_c),
            name(*expected_c),
            count,
            f1,
            ...
            f2,
            {
                let r: f64 = spirix_op(o, s1, s2).into();
                format!("{:.3e}", r)
            },
            f64_op(o, f1, f2)
        );
