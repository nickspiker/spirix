...
//!   negatives cover magnitudes (2^(e-1), 2^e]   — 2^31 patterns
//!     (neg_one_normal @ e hits the top exactly)
//!
//! So exp=0 covers magnitudes [0.5, 1.0] ENTIRELY (negatives include -1, positives
//! stop just below +1). exp=1 covers [1.0, 2.0] ENTIRELY (positives include +1).
//! Exponents 0 and below are ALL |v| ≤ 1; exponents 1 and above are ALL |v| ≥ 1.
//!
//! With exp in [-127, 127]:
//!   "≤ 1" exponents: e in [-127, 0] → 128 exponents
...
fn main() {
    ...
    // Below 1 strictly (|v| < 1):
    //   exp=-127..-1 (127 exps): all 2^32 patterns each
    //   exp=0 : all 2^32 except neg_one (which is = 1)
    let below_1: u64 = 127 * per_exp + (per_exp - 1);

    // Equal to 1:
    //   neg_one_normal @ exp=0 = -1
    //   pos_one_normal @ exp=1 = +1
    let eq_1: u64 = 2;

    // Above 1 strictly (|v| > 1):
    //   exp=1 : all 2^32 except pos_one (which is = 1)
    //   exp=2..127 (126 exps)
