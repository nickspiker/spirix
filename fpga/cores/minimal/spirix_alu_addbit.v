...
module spirix_alu_addbit #(
    ...
    // AMBIG exponent (min signed value for active width)
    wire signed [MAX_EXP-1:0] ambig_ei =
        (exp_width == 2'd0) ? -64'sd128 :
        (exp_width == 2'd1) ? -64'sd32768 :
        (exp_width == 2'd2) ? -64'sd2147483648 :
    ...
    // AMBIG detection (LSB-aligned, width-dependent)
    wire a_is_ambig = (a_exp == ambig_ei);
    wire b_is_ambig = (b_exp == ambig_ei);
    ...
    wire b_ei_m1_ambig = (b_ei_m1 == ambig_ei);
    ...
    wire signed [MAX_EXP-1:0] max_ei = ~ambig_ei;       // max valid exp for active width
    wire neg_b_ei_p1_overflow = (b_ei == max_ei);        // b_ei at max, +1 exceeds range
    wire signed [MAX_EXP-1:0] neg_b_ei_normal =
        b_is_pos_half ? b_ei_m1 :
        b_is_neg_one  ? (neg_b_ei_p1_overflow ? ambig_ei : b_ei_p1) :
                        b_ei;
    ...
    always @(*) begin
        ...
        if (is_bitwise && any_non_normal && a_undef) begin
            ...
            if (and_sc_esc_esc) begin
                sc_bitwise = 1'b1; sc_bw_frac = UNDEF_AND; sc_bw_ei = ambig_ei;
            end else if (and_sc_any_zero) begin
                sc_bitwise = 1'b1; sc_bw_frac = {MAX_FRAC{1'b0}}; sc_bw_ei = ambig_ei;
            end else if (and_sc_a_van) begin
                sc_bitwise = 1'b1;
                if (a_is_neg) begin sc_bw_frac = b_frac; sc_bw_ei = b_ei; end
                else          begin sc_bw_frac = {MAX_FRAC{1'b0}}; sc_bw_ei = ambig_ei; end
            ...
            if (or_sc_esc) begin
                sc_bitwise = 1'b1; sc_bw_frac = UNDEF_OR; sc_bw_ei = ambig_ei;
            ...
            if (xor_sc_esc) begin
                sc_bitwise = 1'b1; sc_bw_frac = UNDEF_XOR; sc_bw_ei = ambig_ei;
    ...
    always @(*) begin
        ...
        if (is_and) begin
            ...
            else              begin bw_negl_frac = {MAX_FRAC{1'b0}}; bw_negl_ei = ambig_ei; end
        end else if (is_or) begin
    ...
    // LSB-aligned AMBIG for S2
    wire signed [MAX_EXP-1:0] s2_ambig_ei =
        (s2_exp_width == 2'd0) ? -64'sd128 :
        (s2_exp_width == 2'd1) ? -64'sd32768 :
        (s2_exp_width == 2'd2) ? -64'sd2147483648 :
                                  64'sh8000000000000000;
    ...
    // Close underflow: result exp at or below AMBIG (65-bit signed comparison)
    wire close_underflow = $signed(close_exp_wide) <= $signed({s2_ambig_ei[MAX_EXP-1], s2_ambig_ei});
    // Close overflow: result exp exceeds max for active width (127 for E3, etc.)
    wire close_overflow = $signed(close_exp_wide) > $signed({~s2_ambig_ei[MAX_EXP-1], ~s2_ambig_ei});
    ...
    // Far underflow: result exp at or below AMBIG (65-bit signed comparison)
    wire far_underflow = $signed(far_exp_wide) <= $signed({s2_ambig_ei[MAX_EXP-1], s2_ambig_ei});
    // Far overflow: result exp exceeds max for active width
    wire far_overflow = $signed(far_exp_wide) > $signed({~s2_ambig_ei[MAX_EXP-1], ~s2_ambig_ei});
    ...
    wire signed [MAX_EXP-1:0] result_ei =
        ...
        s2_bypass_sub         ? s2_neg_big_ei :
        s2_bw_negligible      ? s2_bw_negl_ei :
        (path_is_zero | path_underflow | path_overflow) ? s2_ambig_ei :
                                path_ei;
