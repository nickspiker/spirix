// Exhaustive testbench for spirix_subtract (parameterized).
// Uses F5E4 (5-bit frac, 4-bit exp) for full enumeration.
// Gold model: independently negate b with N1 corner-case handling,
// then call single-path addition gold model at DUT precision.
...
module tb;
    ...
    spirix_subtract #(.FRAC_BITS(FRAC),.EXP_BITS(EXP)) dut(
    ...
    // Gold model: single-path add at DUT precision (same as test_spirix_add.v)
    task gold_add;
    ...
    // Gold model: subtract (negate b, then add)
    ...
    task gold_sub;
        ...
        output signed [FRAC-1:0] of_;
        output signed [EXP-1:0]  oe;
        reg signed [FRAC-1:0] neg_bf;
        reg signed [EXP-1:0]  neg_be;
        begin
            if (bf == POS_HALF) begin
                neg_bf = NEG_ONE;
                neg_be = be - 1;
            end else if (bf == NEG_ONE) begin
                neg_bf = POS_HALF;
                neg_be = be + 1;
            end else begin
                neg_bf = -bf;
            ...
            gold_add(af, neg_bf, ae, neg_be, of_, oe);
        end
    endtask
