...
enum Op { Floor, Ceil, Round, Frac }

fn op_name(o: Op) -> &'static str {
    match o { Op::Floor => "floor", Op::Ceil => "ceil", Op::Round => "round", Op::Frac => "frac" }
}

fn spirix_op(o: Op, a: S) -> S {
    match o { Op::Floor => a.floor(), Op::Ceil => a.ceil(), Op::Round => a.round(), Op::Frac => a.frac() }
}
    ...
    for op in [Op::Floor, Op::Ceil, Op::Round, Op::Frac] {
        ...
        for f in -128i8..=127 {
            for e in -128i8..=127 {
                ...
                if !s.is_normal() { continue; } // let's keep the oracle check to normals only
